...
const GstPurchase = () => {
  ...
  const addItem = () => {
    if (products.length === 0) {
      toast.error("No GST products available");
      return;
    }
    ...
    const newItem: PurchaseItem = {
      ...
      gst_rate: products[0].default_gst_rate || 0,
  ...
  const updateItem = (index: number, field: keyof PurchaseItem, value: any) => {
    ...
    if (field === "product_id") {
      ...
      if (product) {
        ...
        updated[index].gst_rate = product.default_gst_rate || 0;
      }
    }
  ...
  const calculateTotals = () => {
    ...
    const gstTotal = items.reduce((sum, item) => {
      return sum + (item.line_total * item.gst_rate) / 100;
    }, 0);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const purchaseNumber = `GP-${new Date().getFullYear()}-${Date.now()}`;
      ...
      <header className="border-b bg-card shadow-sm">
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-bold text-foreground">GST Purchase</h1>
              <p className="text-sm text-muted-foreground">Create a new GST purchase invoice</p>
            </div>
          </div>
        </div>
      </header>
